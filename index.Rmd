  ...
  ggplot(aes(x=Country, y = FemaleEmploymentIndustry))+
     geom_segment(aes(x=Country, xend=Country, y=0, yend=FemaleEmploymentIndustry), color = "blue")+
      geom_point(color = "#0066ff", size = 4) + coord_flip() + ggtitle("Part de femmes employées dans l'industrie dans les 15 pays au PIB le plus élevé en 2016")+
        xlab("Pays") + ylab("Pourcentgae de femmes employées travaillant dans l'industrie") + theme_ipsum(plot_title_size = 8)
```
...
datatriDiffEspVie%>%
  ggplot(aes(x=Country, y =diffEspVie))+
     geom_segment(aes(x=Country, xend=Country, y=0, yend=diffEspVie), color = "blue")+
      geom_point(color = "#0066ff", size = 4) + coord_flip() + ggtitle("Différence d'espérance de vie entre femmes et hommes pour les 15 pays avec les PIB les plus élevés")+
        xlab("Pays") + ylab("Différence d'espérance de vie entre femmes et hommes") + theme_ipsum()
```
...
don=xts(x = xdata[,-1], order.by = xdata$y)#On crée un format xts
dygraph(don, main= "Espérance de vie au Buhtan entre 1960 et 2016", xlab = "Années", ylab = "Espérance de vie")
```
